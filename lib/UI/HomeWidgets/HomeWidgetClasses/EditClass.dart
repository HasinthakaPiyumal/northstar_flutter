...
class EditClass extends StatelessWidget {
  EditClass({required this.classData});
  ...
  Widget build(BuildContext context) {
    void saveClass() async {
      ...
      Map res = await httpClient.editClass(data);
      if (res['code'] == 200) {
        ready.value = true;
        Get.back();
        print(res);
        showSnack("Success", "Class Updated");
      } else {
        ready.value = true;
        showSnack("Class Update Failed", res['data']['info']['message']);
      }
    }
    void processData(){
      name.text = classData['class_name'];
      description.text = classData['description'];
      dateTimeObj = DateTime.parse(classData['shedule_time']);
      datetime.text = DateFormat('yyyy-MM-dd HH:mm a').format(DateTime.parse(classData['shedule_time'])).toString();
      type.value = int.parse(classData['class_type']);
      locationOrLink.text = classData['location'];
    }
    processData();
            ...
            children: [
                ...
                onTap: () {
                  DatePickerBdaya.showDateTimePicker(
                    ...
                    onConfirm: (date) {
                      ...
                      datetime.text = DateFormat('yyyy-MM-dd HH:mm a')
                          .format(date)
                          .toString();
                      dateTimeObj = date;
                    },
              ...
              Obx(
                () => TextField(
                  ...
                  decoration: InputDecoration(
                      prefixIcon: Icon(type.value == 1
                          ? Icons.location_on_outlined
                          : Icons.http_sharp),
                      border: UnderlineInputBorder(),
                      label: Text(type.value == 1 ? 'Location' : 'Link')),
                  onSubmitted: (value) {
                    // datetimeNode.requestFocus();
                  },
                ),
